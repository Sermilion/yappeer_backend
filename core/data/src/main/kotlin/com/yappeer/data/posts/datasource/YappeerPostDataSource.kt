    ...
    return transaction {
      try {
        val existingLike = PostLikesDislikesTable
          .selectAll().where { (PostLikesDislikesTable.postId eq postId) and (PostLikesDislikesTable.userId eq userId) }
          .firstOrNull()
    ...
    transaction {
      PostTable.update({ PostTable.id eq postId }) {
        it[likes] = PostLikesDislikesTable.selectAll().where {
          (PostLikesDislikesTable.postId eq postId) and (PostLikesDislikesTable.likeStatus eq LikeStatus.Like.value)
        }.count().toInt()
        it[dislikes] = PostLikesDislikesTable.selectAll().where {
          (PostLikesDislikesTable.postId eq postId) and (PostLikesDislikesTable.likeStatus eq LikeStatus.Dislike.value)
        }.count().toInt()
      }
    }
  }
